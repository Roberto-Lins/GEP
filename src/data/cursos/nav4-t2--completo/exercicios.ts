...
import { questoesPreparacao } from '../_familias/nav4-t2/questoes';
import matriz from '../_familias/nav4-t2/matriz-cobertura.json';
import { questoesDaModalidade } from '@utils/study-mode-questions';
const modalidade = 'completo' as const;
const conceitosEnsinados = new Set(matriz.conceitos.filter((c) => c.presenca[modalidade]).map((c) => c.concept_id));
export const todasQuestoes: Questao[] = questoesDaModalidade(questoesPreparacao, modalidade, conceitosEnsinados);
